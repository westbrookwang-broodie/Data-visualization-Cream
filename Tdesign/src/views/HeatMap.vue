...
export default {
  ...
  async mounted() {
    ...
    for(let i=0;i<=379;i++){
      let tmp = []
      tmp.push(i%19)
      tmp.push(math.floor(i/19))
      tmp.push(math.ceil(data2[i].movie_vote_var))
      this.data.push(tmp)
    }
...
//       });
    ...
    option = {
      ...
      visualMap: {
        ...
        width: "700px",
        inRange:{
          color: ['rgba(38,46,241,0.5)','rgb(41,217,103)', 'rgb(234,59,15)']
        }
      },
      series: [
        {
...
async function icsv() {
  ...
  data2.sort((a, b)=>{
    return +b.movie_vote - +a.movie_vote
  })
