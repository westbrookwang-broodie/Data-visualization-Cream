...
class Spider:
    ...
    def check_length_same(self, datalist):
        b = len(set(datalist))
        if b > 1:
            print('invalid page: ' + datalist, file=self.log)
            return -1
        else:
            return datalist[0]

    def run(self):
        self.log = open('../logs/spider_log.txt', mode='a', encoding='utf-8')
        print("-----Spider begin-----", file=self.log)
        ...
        for ts in range(2):
            ...
            for film_info in id_list:
                ...
                #     continue
                finish_task += 1
                print("Begin task: %d" % finish_task, file=self.log)
                output_path = self.filepath.format(film_info[0])
                print("Film id: %s   Film name: %s" % (film_info[0], film_info[1]), file=self.log)
                ...
                for url in url_list:
                    ...
                    if length >= 0:
                        ...
                        print("page %d finish" % (t // 20), file=self.log)
                    t += length
                ...
                end_time = time.time()
                print("-----Spider of film %s finish at %f s-----" % (film_info[0], end_time - start_time), file=self.log)
                print("共 %d 条短评" % t, file=self.log)
                print("共 %d 条短评" % t)
                print("Finished task: %d" % finish_task, file=self.log)
        self.log.close()
