...
function generate(ts, vueCompilerOptions, sourceTemplate, sourceLang, templateAst, hasScriptSetupSlots, cssScopedClasses = []) {
    ...
    function generateComponentVars() {
        ...
        for (const tagName in tagNames) {
            ...
            codes.push(`& import('./__VLS_types.d.ts').WithComponent<'${varName}', typeof __VLS_components, `, 
    ...
    function visitElementNode(node, parentEl, componentCtxVar) {
        ...
        if (isNamespacedTag) {
            codes.push(`const ${componentVar} = (await import('./__VLS_types.d.ts')).asFunctionalComponent(${tag}, new ${tag}({`, ...createPropsCode(node, props, 'slots'), '}));\n');
        }
        else if (dynamicTagExp) {
            const dynamicTagVar = `__VLS_${elementIndex++}`;
            codes.push(`const ${dynamicTagVar} = `, ...createInterpolationCode(dynamicTagExp.loc.source, dynamicTagExp.loc, dynamicTagExp.loc.start.offset, capabilitiesPresets.all, '', ''), ';\n');
            codes.push(`const ${componentVar} = (await import('./__VLS_types.d.ts')).asFunctionalComponent(`, `${dynamicTagVar}, `, `new ${dynamicTagVar}({`, ...createPropsCode(node, props, 'slots'), '}));\n');
        }
        else {
            codes.push(`const ${componentVar} = (await import('./__VLS_types.d.ts')).asFunctionalComponent(`, `__VLS_templateComponents['${componentVars[tag] ?? tag}'], `, `new __VLS_templateComponents['${componentVars[tag] ?? tag}']({`, ...createPropsCode(node, props, 'slots'), '}));\n');
        }
        for (const offset of tagOffsets) {
            if (isNamespacedTag) {
                codes.push([tag, 'template', [offset, offset + tag.length], capabilitiesPresets.all], ';\n');
            }
            else if (dynamicTagExp) {
                continue;
            }
    ...
    function generateEvents(node, componentVar, componentInstanceVar, componentCtxVar) {
        for (const prop of node.props) {
            if (prop.type === 7 /* CompilerDOM.NodeTypes.DIRECTIVE */
                ...
                codes.push(`let ${eventVar} = { '${prop.arg.loc.source}': `, `(await import('./__VLS_types.d.ts')).pickEvent(${componentCtxVar}.emit!, '${prop.arg.loc.source}' as const, (await import('./__VLS_types.d.ts')).componentProps(${componentVar}, ${componentInstanceVar})`, ...createPropertyAccessCode([
    ...
    function createPropsCode(node, props, mode, propsFailedExps) {
        ...
        for (const prop of props) {
            if (prop.type === 7 /* CompilerDOM.NodeTypes.DIRECTIVE */
                ...
                && !prop.arg
                && prop.exp?.type === 4 /* CompilerDOM.NodeTypes.SIMPLE_EXPRESSION */) {
                codes.push('...', ...createInterpolationCode(prop.exp.content, prop.exp.loc, prop.exp.loc.start.offset, getCaps(capabilitiesPresets.all), '(', ')'), ', ');
                const fb = getFormatBrackets(formatBrackets.normal);
                if (fb) {
    ...
    function generateDirectives(node, componentVar) {
        for (const prop of node.props) {
            if (prop.type === 7 /* CompilerDOM.NodeTypes.DIRECTIVE */
                ...
                ], ')', '(', ['', 'template', prop.loc.start.offset, capabilitiesPresets.diagnosticOnly], componentVar, ['', 'template', prop.loc.start.offset + 'v-'.length + prop.name.length, capabilitiesPresets.diagnosticOnly], ', ');
                if (prop.exp?.type === 4 /* CompilerDOM.NodeTypes.SIMPLE_EXPRESSION */) {
