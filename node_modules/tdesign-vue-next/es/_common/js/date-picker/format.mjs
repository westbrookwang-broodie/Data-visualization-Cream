...
import { c as commonjsGlobal } from '../../../_chunks/dep-bec45e18.mjs';
...
import '../../../_chunks/dep-662d3380.mjs';
...
function formatTime(value, format, timeFormat, defaultTime) {
  ...
  if (Array.isArray(value)) {
    ...
    result = value.map(function (v, i) {
      return v ? dayjs(v, format).format(timeFormat) : calcFormatTime(defaultTime[i], timeFormat);
    });
    result = result.length ? result : defaultTime.map(function (t) {
      return calcFormatTime(t, timeFormat);
    });
  } else {
    result = value ? dayjs(value, format).format(timeFormat) : calcFormatTime(defaultTime, timeFormat);
  }
  return result;
}
...
export { TIME_FORMAT, calcFormatTime, formatDate, formatTime, getDefaultFormat, initYearMonthTime, isValidDate, parseToDayjs };
