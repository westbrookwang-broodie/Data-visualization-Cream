...
function formatTime(value, format, timeFormat, defaultTime) {
  ...
  if (Array.isArray(value)) {
    ...
    result = value.map(function (v, i) {
      return v ? dayjs__default["default"](v, format).format(timeFormat) : calcFormatTime(defaultTime[i], timeFormat);
    });
    result = result.length ? result : defaultTime.map(function (t) {
      return calcFormatTime(t, timeFormat);
    });
  } else {
    result = value ? dayjs__default["default"](value, format).format(timeFormat) : calcFormatTime(defaultTime, timeFormat);
  }
  return result;
}
